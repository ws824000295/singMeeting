	...
	var dragJson = {
	 ...
	 /*           if(type == "img"){			//图片拖动完 改变手机框高
		            ...
		            if(divTop > 486 && divTop > editHeigh){
			            $(".phone_content_children").css("height",divTop); 
		            }
	        ...
	        drag:function(e,ui){
	    	 ...
	    	 if(ui.position.left>=xarea){
			   ...
			   if($(".phone_content .columnSelect").length>1){
				 ...
				 $.each(columnArr,function(i,n){
				  ...
				  var be=$(".phone_content_children").width()+"px",
				  ...
				  /*留2线*/
				  $(".phone_content").append(xian2);
				  $(".phone_content").append(xian1);
	        },
	        start:function(e,ui){
...
function bindResizable(obj){
	comp_resizable(obj,{'containment': '.phone_content'});
} 
