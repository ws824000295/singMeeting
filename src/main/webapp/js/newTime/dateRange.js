...
*/
    ...
    // First 7 Days, First All
    this.periodObj[__method.mOpts.aFirst7Days] = -7;
    this.periodObj[__method.mOpts.aAllDays] = -9999;
    // 记录初始默认时间
    this.startDefDate = '';
    // 随机ID后缀
...
/**
 * 设置新的最小时间
 * @param  {obj} timeObj minvalidate
 * @return {none}         null
 */
pickerDateRange.prototype.jsReinit = function(timeObj) {
    var __method = this;
    __method.mOpts.minValidDate = timeObj.minValidDate;
    __method.init();
    __method.show(false, __method);
}
/**
 * 设置新的时间段
 * @param  {obj} dateRange {startDate: '2016-12-12', endDate: '2019-11-12'}
 * @return {bool}           设置成功还是失败
 * by chenjsh36
 */
pickerDateRange.prototype.jsSetNewDate = function(timeObj) {
    var __method = this;
...
pickerDateRange.prototype.getSpecialPeriod = function(period){
    ...
    // Add first7Days 发布后7天
    var js_startTime = new Date(__method.mOpts.minValidDate * 1000);    
    var js_todayStr = js_startTime.getFullYear() + '-' + (js_startTime.getMonth() + 1) + '-' + js_startTime.getDate();
    if (period == -7) {
        js_startTime.setTime(js_startTime.getTime() + (6 * 24 * 60 * 60 * 1000));
        var _now = new Date();
        var _nowFormat = new Date(_now.getFullYear() + '-' + (_now.getMonth() + 1) + '-' + (_now.getDate()) );
        if (js_startTime.getTime() > _nowFormat.getTime()) {
            js_startTime = _nowFormat;            
        }
        // 发布后7天
        var js_otherdayStr = js_startTime.getFullYear() + '-' + (js_startTime.getMonth() + 1) + '-' + js_startTime.getDate();
        return {today: js_todayStr, otherday: js_otherdayStr};
    }
    // Add firstAll 
    if (period == -9999) {
        var js_now = new Date();
        return {today: js_todayStr, otherday: ('' + js_now.getFullYear() + '-' + (js_now.getMonth() + 1) + '-' + js_now.getDate())};
    }
    ...
    // Debug by chenjsh36 at 2016-0414
    // While we rewrite the id of aYesterday, the following code would be error for could not set todayStr as otherdayStr
    // Old code :
    // if(period == __method.periodObj.aYesterday){
	if(period == __method.periodObj[__method.mOpts.aYesterday]){
...
pickerDateRange.prototype.close = function(btnSubmit) {
    ...
    $("#" + this.inputId).removeClass('focus');
    return false;
};
