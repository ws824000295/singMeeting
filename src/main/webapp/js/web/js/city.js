...
//省市区街道地址选择四级联动
 var currentProvince = {};                                               //当前选择的省级
 var  currentCity = {};                                                //当前选择的市级
 var  currentArea = {};    
            ...
            city: function () {
                ...
                try {
                    ...
                    if (!len) {
                       ...
                       // $city.prepend('<li  id="allProvice">' +currentProvince.name + '</li>');
				        $city.prepend('<li  id="allProvice">' + "全省"+ '</li>');
                        // 选择城市
                        $city.find("li")
                            .click(function() {
                                var $this = $(this);
        ...
        var loadDataHandler = {
            ...
            townData: function (callback) {
                if (currentArea && currentArea.code) {
                    ...
                    $.ajax({
                        url: 'https://passer-by.com/data_location/town/' + currentArea.code + '.json',
                        dataType: 'json',
                    ...
                    case 1:   //省级
                        {
                            ...
                            if(currentProvince.code ===undefined){
							//  alert("allProvice");
							//  alert($("#lastprovince").text());
                              $container.hide();
							  $("#Addr").text($("#lastprovince").text());
							//  toolHanlder.close();
                              query();
                    ...
                    case 2: //市级
                        {
							...
							if(currentCity.code===undefined){
							//   alert("allCity");
								   $container.hide();
							   $("#Addr").text($("#lastcity").text());
							   query();
							   //toolHanlder.close();
                    ...
                    case 3://区县
                        {
						    ...
						    if(currentArea.code===undefined){
							//  alert("allArea ");
						      $container.hide();
							  $("#Addr").text($("#lastarea").text());
                              query();
							  //toolHanlder.close();
            ...
            createHtml: function (level,para) {  // lxj, para 在后面调用时，使用 town(一个对象，存储了全部乡镇街道) 实参传入
                switch (level) {
                    case 1:
                        {
                            $province.empty();
						     ...
						     $province.append('<li  id="allProvice">' + "全国"+ '</li>');
                            //组装省级选择项
                    ...
                    case 3:
                        {
                            $area.empty();
                        //    $area.append('<li id="allArea">' + "全市" + '</li>');
                            for (var i in areas) {
                            ...
                            if($area.find("li").length != 0){
   							 ...
   							 $area.prepend('<li id="allArea">' + "全市" + '</li>');
   							}
                        } break;
                    case 4:
                        {
                            if (para) {
							    $town.append('<li id="allTown">' + "全县区" + '</li>');
